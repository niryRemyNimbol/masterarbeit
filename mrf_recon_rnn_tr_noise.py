...
#One percent gaussian noise data + test different noise levels
series_mag_1 = np.abs(D.T[permutation] + 0.01 * np.max(np.real(D)) * np.random.normal(0.0, 1.0, D.T.shape) + 1j * 0.01 * np.max(np.imag(D)) * np.random.normal(0.0, 1.0, D.T.shape)).T
series_mag_1 /= np.linalg.norm(series_mag_1, axis=0)
series_mag_5 = np.abs(D.T[permutation] + 0.05 * np.max(np.real(D)) * np.random.normal(0.0, 1.0, D.T.shape) + 1j * 0.05 * np.max(np.imag(D)) * np.random.normal(0.0, 1.0, D.T.shape)).T
series_mag_5 /= np.linalg.norm(series_mag_5, axis=0)
series_mag_10 = np.abs(D.T[permutation] + 0.1 * np.max(np.real(D)) * np.random.normal(0.0, 1.0, D.T.shape) + 1j * 0.1 * np.max(np.imag(D)) * np.random.normal(0.0, 1.0, D.T.shape)).T
series_mag_10 /= np.linalg.norm(series_mag_10, axis=0)
#series_mag /= np.amax(series_mag, axis=0)
series_mag_1 = series_mag_1.T
series_mag_5 = series_mag_5.T
series_mag_10 = series_mag_10.T
...
train_set_1 = [series_mag_1[batch_size*step:batch_size*(step+1)].reshape((batch_size, timesteps, num_in_fc), order='F') for step in range(batches_per_epoch)]
train_set_1.append(series_mag_1[batch_size*batches_per_epoch:train_size].reshape((train_size - batch_size*batches_per_epoch, timesteps, num_in_fc), order='F'))
val_set_1 = series_mag_1[train_size:train_size+val_size].reshape((val_size, timesteps, num_in_fc), order='F')
train_set_5 = [series_mag_5[batch_size*step:batch_size*(step+1)].reshape((batch_size, timesteps, num_in_fc), order='F') for step in range(batches_per_epoch)]
train_set_5.append(series_mag_5[batch_size*batches_per_epoch:train_size].reshape((train_size - batch_size*batches_per_epoch, timesteps, num_in_fc), order='F'))
val_set_5 = series_mag_5[train_size:train_size+val_size].reshape((val_size, timesteps, num_in_fc), order='F')
train_set_10 = [series_mag_10[batch_size*step:batch_size*(step+1)].reshape((batch_size, timesteps, num_in_fc), order='F') for step in range(batches_per_epoch)]
train_set_10.append(series_mag_10[batch_size*batches_per_epoch:train_size].reshape((train_size - batch_size*batches_per_epoch, timesteps, num_in_fc), order='F'))
val_set_10 = series_mag_10[train_size:train_size+val_size].reshape((val_size, timesteps, num_in_fc), order='F')
...
#                    batch_y = relaxation_times[(step-1)%32 * batch_size:min(((step-1)%32+1) * batch_size, series_mag.shape[0])]
        for k in range(len(train_set_1)):
            batch_x_1 = train_set_1[k]
            batch_x_5 = train_set_5[k]
            batch_x_10 = train_set_10[k]
...
#        val_loss_writer.add_summary(val_loss_sum, step)
        ...
        if epoch % display_step == 1:
            print("Epoch " + str(epoch) + ", Validation Loss= " + "{:.10f}".format(val_loss_1[-1]))
            print("Epoch " + str(epoch) + ", Validation Loss= " + "{:.10f}".format(val_loss_5[-1]))
        ...
        if epoch == 1:
            ...
            if val_loss_1[-1] < best_loss:
                ...
                counter += 1
        if counter > 20:
            ckpt_file_1 = ckpt_dir + 'model_var_tr_norm_1_checkpoint{}.ckpt'.format(epoch)
