...
dict_t1 = [1, 2, 4]
dict_t2 = [1, 2, 4]
dict_ = [(t1, t2) for t1 in dict_t1 for t2 in dict_t2]
...
for n in range(len(dict_)):
    with tf.variable_scope('density_t1{}_t2{}'.format(dict_[n][0], dict_[n][1])):
        step_t2 = int(200/dict_[n][0])
        D = dictionary.D[:, ::dict_[n][0]]
        lut = dictionary.lut[:2, ::dict_[n][0]]
        D = np.concatenate([D[:, begin::dict_[n][1]*step_t2] for begin in range(step_t2)], axis=1)
        lut = np.concatenate([lut[:, begin::dict_[n][1]*step_t2] for begin in range(step_t2)], axis=1)
...
# Run the initializer
            ...
            val_loss_writer = tf.summary.FileWriter('tensorboard/validation_loss_density_t1{}_t2{}/'.format(dict_[n][0], dict_[n][1]), sess.graph)
            counter = 0
            total_loss = 0
...
#        val_loss_writer.add_summary(val_loss_sum, step)
        ...
        # Reshuffling the train set
                ...
                if epoch == 1:
                    ckpt_file = ckpt_dir + 'model_fc_density_t1{}_t2{}_checkpoint{}.ckpt'.format(dict_[n][0], dict_[n][1], 0)
                    saver.save(sess, ckpt_file)
                    best_loss = val_loss
                elif epoch % early_stop_step == 0:
                    if val_loss < best_loss:
                        best_loss = val_loss
                        prev_ckpt = ckpt_dir + 'model_fc_density_t1{}_t2{}_checkpoint{}.ckpt'.format(dict_[n][0], dict_[n][1], epoch-10*(counter+1))
                        ckpt_file = ckpt_dir + 'model_fc_density_t1{}_t2{}_checkpoint{}.ckpt'.format(dict_[n][0], dict_[n][1], epoch)
                        saver.save(sess, ckpt_file)
